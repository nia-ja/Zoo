...
async def main():
    screen_width, screen_height, block_size = get_screen_size()
        
    print(f"width: {screen_width}, height: {screen_height}, block_size: {block_size}")

    cols = math.floor(screen_width / block_size)
    rows = math.floor(screen_height / block_size) - 1
    ...
    elephant_pack = pack.get_pack(obj_number + 1)
    ...
    global screen, clock, window_hight, window_width, font_title, font_big, font_small, iterations, title
    # pygame setup
    window_hight = my_grid.y*block_size
    window_width = my_grid.x*block_size
    ...
    while running:
        ...
        if iterations < 1:
            ...
            drawObj(my_grid.get_coordinates(), my_grid.get_all_empty_xy())
